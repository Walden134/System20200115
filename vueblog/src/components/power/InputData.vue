...
export default {
  ...
  methods: {
    ...
    getExpectOutputHeadCurve(data) {
      let i = 0;
      this.hydrostation.expectOutputHeadCurve.curveData[0] = [];
      this.hydrostation.expectOutputHeadCurve.curveData[1] = [];
      data.map(val => {
        this.hydrostation.expectOutputHeadCurve.curveData[0][i] = val["水位"];
        this.hydrostation.expectOutputHeadCurve.curveData[1][i] =
          val["下泄流量"];
        i++;
      });
    },
