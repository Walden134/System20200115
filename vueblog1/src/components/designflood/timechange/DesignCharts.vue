      ...
      bus.$on("designp", data => {
        if (data.length > 0) {
          for (let j = 0; j < data[0].length; j++) {
            ...
            this.$refs.dschart.options.series[3].data[j][1] = data[4][j];
          }
        } else {
    ...
    inintChartData() {
      ...
      if (data2.length > 0) {
        for (let j = 0; j < data2[0].length; j++) {
          ...
          this.$refs.dschart.options.series[3].data[j][0] = data2[0][j];
          ...
          this.$refs.dschart.options.series[3].data[j][1] = data2[4][j];
        }
      }
      let Pchart = echarts.init(document.getElementById("designChart"));
      Pchart.setOption(this.chartdata, true);
